...
-- Back-fill existing rows with a placeholder document key to satisfy future unique constraint
-- (Existing rows are pre-verification records with no business context — leave them as-is)
...
-- Using IF NOT EXISTS equivalent via DO block
DO $$
BEGIN
  IF NOT EXISTS (
